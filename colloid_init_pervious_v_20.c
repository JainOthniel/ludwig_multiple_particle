...
double colloid_calc_vf(cs_t * cs, const double ah, const int nb);
...
int main(int argc, char ** argv) {
  ...
  const double r0_default = 2.3;   /* robot radius */
...
/* added an if statement to separate robot and obsatcle  n =0 means robot */
...
double colloid_calc_vf(cs_t * cs, const double ah, const int nb) {
  ...
  //printf("ntotal[x] = %d, ntotal[Y] = %d, ntotal[Z] = %d\n", ntotal[X], ntotal[Y], ntotal[Z]);
  //n = vf*volume/(4.0*pi*ah*ah*ah/3.0);
  vf = nb*(4.0*pi*ah*ah*ah/3.0) / volume;
...
int colloid_init_mc(cs_t * cs, int nc, colloid_state_t * state, double dh) {
  ...
  if (nc > nbcc){
   ...
   new_vf = colloid_calc_vf(cs, ah_ref, nbcc);
